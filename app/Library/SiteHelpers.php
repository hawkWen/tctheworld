...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function tranHistory( $user_id, $balance_before, $balance_after, $balance_change, $transaction_detail, $status, $request)
	{

		$username = \DB::table('tb_users')->where('id', $user_id)->first();
		$data = array(
			'user_id' => $user_id,
		...
		\DB::table('transac_histories')->insert($data);		
...
static public function buttonAction( $module , $access , $id , $setting)
	{
		...
		if($access['is_detail'] ==1) {
			if($setting['view-method'] != 'expand')
			{
				...
				$html .= '<a href="'.url($module.'/'.$id).'" '.$onclick.' class="tips btn btn-info btn-sm " > <i class="icon-eye"></i> </a>';
			}
		}
		if($access['is_edit'] ==1) {
			$onclick = " onclick=\"ajaxViewDetail('#".$module."',this.href); return false; \"" ;
			...
			$html .= '<a href="'.url($module.'/'.$id).'/edit" '.$onclick.'  class="tips btn btn-primary btn-sm">  <i class="icon-note"></i> </a>';
		}
		$html .= '</div></div>';
		return $html;
	}	
	...
	static public function bfregister( $newuserid, $password , $firstname, $tel)
	{
			$endpoint = "https://api.bfx.fail/v4/user/register";
			$client = new \GuzzleHttp\Client();


			$response = $client->request( 'POST', $endpoint, [
				  'headers' => [
			        'x-api-betflix' => 'OrmI8qdUfXzB7F05',
			        'x-api-key'     => 'c701afc642206a07dc5bd30fb7a7c0dc'
			      ],
				  'form_params' => [
				    'username' => $newuserid, 
				    'password' => $password,
			...
			if($statusCode==200){
				...
				if($content['status']=='success'){
					...
					return $content['error_code'].':'.$content['msg'];
				}
	...
	static public function bfgetbalance($userbalance = 0)
	{

			$balance = 0;
			if($userbalance!=0){
				$userid = $userbalance;
				$getuser = \DB::table('tb_users')->where('id', $userid)->first();
				$gameusername = $getuser->gameusername;
				...
				$endpoint = "https://api.bfx.fail/v4/user/balance";
				$client = new \GuzzleHttp\Client();
				...
				if($statusCode==200){
					...
					$content = json_decode($response->getBody(), true);
						...
						if($content['status']=='success'){
							...
							$balance = $content['data']['balance'];
							
							\DB::table('tb_users')->where('id', $userid)->update(['balance' => $balance]);
	...
	static public function bflogin_token()
	{
		$userid = \Auth::user()->id;
		$endpoint = "https://api.bfx.fail/v4/play/login";
				$client = new \GuzzleHttp\Client();
				...
				if($statusCode==200){
					...
					$content = json_decode($response->getBody(), true);
						...
						if($content['status']=='success'){
							return $content['data']['login_token'];
						}else{
							return 'error';
						}
				}
	...
	static public function bftransfer($user_id,$amount,$ref,$type,$request)
	{
		$balance = 0;
			if($user_id==0){
				$userid = \Auth::user()->id;
		...
		$endpoint = "https://api.bfx.fail/v4/user/transfer";
					...
					$content = json_decode($response->getBody(), true);
						...
						if($content['status']=='success'){
							...
							if($type=='deposit'){
								...
								self::tranHistory( $userid, $content['data']['before_balance'], $balance, $content['data']['amount'], 'เพิ่มเครดิต', 'success', $request);
							}else{
								self::tranHistory( $userid, $content['data']['before_balance'], $balance, $content['data']['amount'], 'ลดเครดิต', 'success', $request);
							}
							...
							if($type=='deposit'){
								self::tranHistory( $userid, $balance, $balance, 0.00, 'เพิ่มเครดิต', 'error', $request);
							}else{
								self::tranHistory( $userid, $balance, $balance, 0.00, 'ลดเครดิต', 'error', $request);
							}
							return 'error';
						}
				}
	...
	static public function allcredit(){
		$endpoint = "https://api.bfx.fail/v4/user/sumCredit?upline=bncb";
				$client = new \GuzzleHttp\Client();
		...
		$statusCode = $response->getStatusCode();
				...
				if($statusCode==200){
					...
					$content = json_decode($response->getBody(), true);
						...
						if($content['status']=='success')
						{
							return $content['data']['total_credit'];
						}

					}
	}

	static public function allbalance(){
		$endpoint = "https://api.bfx.fail/v4/agent/balance?upline=bncb";
				$client = new \GuzzleHttp\Client();
	...
	static public function bfuser_turnover()
	{

		$casino = array("r88", "amb", "gdg", "gd88", "ag", "bg", "sa", "dg", "sexy", "wm");

		$userid = \Auth::user()->id;
		$gameusername = \Auth::user()->gameusername;
		$ref_id = \Auth::user()->ref;
		$datequery =date('Y-m-d');

		$stdatequery =date('Y-m-d');

		$endpoint = "https://api.bfx.fail/v4/report/summary2?username=".$gameusername."&start=".$stdatequery."&end=".$datequery;
				$client = new \GuzzleHttp\Client();
		...
		$typec = 'slot';
				...
				if($statusCode==200){
					...
					$content = json_decode($response->getBody(), true);
						...
						if($content['status']=='success'&&!empty($content['data']))
						{
							foreach ($content['data'] as $key => $value) {

			                    if(in_array($value['provider'],$casino)){
			                     	$tcasino=$tcasino+$value['turnover'];
			                     	$wcasino=$wcasino+$value['winloss'];
			                    }else{ 
			                   		$tslot=$tslot+$value['turnover'];
			                     	$wslot=$wslot+$value['winloss'];
			                    }
								
							}

							$slotcom = (0.3/100)*$tslot;
							$casinocom = (0.3/100)*$tcasino;
						
							$users_turnover = \DB::table('users_turnover')->where('user_id','=',$userid)->where('date','=',$datequery)->first();
							...
							if($users_turnover==''){
								...
								$data = array(
									...
									'slot_com'	=> $slotcom,
									'casino' => $tcasino,
									'casino_com' => $casinocom,
									'slot_winloss' => $wslot,
									'casino_winloss' => $wcasino,
									'total_amount' => $slotcom+$casinocom
								);
								
								\DB::table('users_turnover')->insert($data);	
								...
								\DB::table('users_turnover_daily')->insert($data);		
							}else{
								\DB::table('users_turnover')->where('user_id', $userid)->where('date','=',$datequery)->update(['slot' => $tslot,'slot_com' => $slotcom,'casino' => $tcasino,'casino_com' => $casinocom,'slot_winloss'=>$wslot,'casino_winloss'=>$wcasino,'total_amount'=>$slotcom+$casinocom]);

								\DB::table('users_turnover_daily')->where('user_id', $userid)->where('date','=',$datequery)->update(['slot' => $tslot,'slot_com' => $slotcom,'casino' => $tcasino,'casino_com' => $casinocom,'slot_winloss'=>$wslot,'casino_winloss'=>$wcasino,'total_amount'=>$slotcom+$casinocom,'gameusername' => $gameusername,'ref_id'=>$ref_id]);
							}


							$rolloverlookup = \DB::table('rolloverlookup')->where('user_id','=',$userid)->first();

							$users_return = \DB::table('users_return')->where('user_id','=',$userid)->first();

							if($users_return==''){

								$data = array(
									'user_id' => $userid,
									'total_slot' => $rolloverlookup->slot_com,
									'total_casino' => $rolloverlookup->casino_com,
								);
								
								\DB::table('users_return')->insert($data);

							}else{
								\DB::table('users_return')->where('user_id', $userid)->update(['total_slot' => $rolloverlookup->slot_com,'total_casino' => $rolloverlookup->casino_com]);
							}
