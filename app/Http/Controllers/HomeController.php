...
class HomeController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if ($page != '') {
			...
			$credit_user = \DB::connection('mysql2')->table('tb_users')->where('gameusername', '=', \Auth::user()->gameusername)->first();

			\DB::table('tb_users')->where('gameusername', $gameusername)->update(['balance' => $credit_user->balance,'bonus' => $credit_user->bonus]);
			...
			if ($page == 'promotion') {
				...
				/*if (!empty($request->input('partner')) && $request->input('partner') <> 'all') {
					$games_data = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('partner', '=', $request->input('partner'))->where('game_type', '=', 1)->limit(30)->get();
					$games_count = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->where('partner', '=', $request->input('partner'))->where('game_type', '=', 1)->count();
				} elseif (!empty($request->input('game_type'))) {
					$games_type_data = \DB::connection('mysql2')->table('game_type')->where('id', '=', $request->input('game_type'))->get();
					$games_data = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('game_type', '=', $request->input('game_type'))->limit(30)->get();
					$games_count = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->where('game_type', '=', $request->input('game_type'))->count();
					$this->data['games_type_data'] = $games_type_data;
				} else {
					$games_data = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->limit(30)->orderBy('gameid', 'DESC')->get();
					$games_count = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->count();
				}

				$partners_data = \DB::connection('mysql2')->table('game_partners')->where('active', '=', 1)->get();

				$partners_with_games_data = array();

				foreach ($partners_data as $key => $value) {
					$games = \DB::connection('mysql2')->table('gamelists')->where('partner', '=', $value->short_name)->where('game_type', '=', 1)->limit(4)->orderBy('gameid', 'DESC')->get();
					$partners_with_games_data[$key] =  $value;
					$partners_with_games_data[$key]->games =  $games;
				};

			

				$game_category = \DB::connection('mysql2')->table('game_category')->get();

				$this->data['game_category'] = $game_category;
				$this->data['game_category_count'] = $game_category->count();

				$this->data['games_data'] = $games_data;
				$this->data['games_count'] = $games_count;

			

				$this->data['partners_with_games_data'] = $partners_with_games_data;
				$this->data['partners_data'] = $partners_data;

				$game_type = \DB::connection('mysql2')->table('game_type')->orderBy('id', 'ASC')->get();
				$this->data['game_type'] = $game_type;
				$this->data['game_type_count'] = $game_type->count();


				$game_categories_data = \DB::connection('mysql2')->table('game_category')->get();
				...
				$this->data['req_game_type'] = $request->input('game_type');*/

 						
 
				if(!empty($request->input('partner'))) {
					$this->data['games_data'] = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('partner', '=', $request->input('partner'))->get();
					$gametype = \DB::connection('mysql2')->table('gamelists')->where('partner', '=', $request->input('partner'))->distinct()->get(['game_type']);
					$this->data['gametype'] = $gametype;
					$this->data['partnerdetail'] = \DB::connection('mysql2')->table('game_partners')->where('short_name', '=', $request->input('partner'))->first();
				}

				$this->data['partners_data'] = \DB::connection('mysql2')->table('game_partners')->where('active', '=', 1)->get();
			...
			$this->data['credit_user'] = $credit_user;
			...
			if ($row->template == 'frontend') {
				...
				if ($page == 'games') {
					if(!empty($request->input('partner'))) {
						...
						$page = 'layouts.betflix.games';
					}else{
						$page = 'layouts.betflix.index';
					}
				} else {
					$page = 'layouts.' . config('sximo.cnf_theme') . '.index';
				}
			...
			if (count($sql) >= 1) {
				...
				if(empty(\Auth::user()->gameusername)||\Auth::user()->gameusername==''){
					return redirect('user/login');
				}

				$credit_user = \DB::connection('mysql2')->table('tb_users')->where('gameusername', '=', \Auth::user()->gameusername)->first();
				\DB::table('tb_users')->where('gameusername', \Auth::user()->gameusername)->update(['balance' => $credit_user->balance,'bonus' => $credit_user->bonus]);
				$promotions_current_user = \DB::table('promotions_current_user')
					->join('promotions', 'promotions_current_user.promotion_id', '=', 'promotions.id')
					->select('promotions_current_user.*', 'promotions.promotion_name')
					->where('promotions_current_user.user_id', '=', \Auth::user()->id)
	...
	function loadmoregames(Request $request)
	{
		...
		if (!empty($request->input('partner')) && $request->input('partner') <> 'all') {
			$games_data_more = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('partner', '=', $request->input('partner'))->where('game_type', '=', 1)->offset($lseqno)->limit(9)->get();
		} elseif (!empty($request->input('game_type'))) {
			$games_data_more = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('game_type', '=', $request->input('game_type'))->offset($lseqno)->limit(9)->get();
		} elseif (!empty($request->input('search'))) {
			$games_data_more = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('name', 'like', '%' . $request->input('search') . '%')->limit(30)->get();
		} elseif (!empty($request->input('partner_initial'))) {
			$games_list = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('partner', '=', $request->input('partner_initial'))->where('game_type', '=', 1)->limit(30)->get();
			$games_count = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('partner', '=', $request->input('partner_initial'))->where('game_type', '=', 1)->count();
			$games_data_more = array(
				'count'	=> $games_count,
				'list'	=> $games_list
			);
		} elseif (!empty($request->input('game_type_initial'))) {
			$games_list = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('game_type', '=', $request->input('game_type_initial'))->limit(30)->get();
			$games_count = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->where('game_type', '=', $request->input('game_type_initial'))->count();
			$games_data_more = array(
				'count'	=> $games_count,
				'list'	=> $games_list
			);
		} elseif (!empty($request->input('home_initial'))) {
			$partners_data = \DB::connection('mysql2')->table('game_partners')->where('active', '=', 1)->get();
			$games_data_more = array();

			foreach ($partners_data as $key => $value) {
				$games = \DB::connection('mysql2')->table('gamelists')->where('partner', '=', $value->short_name)->where('game_type', '=', 1)->limit(4)->orderBy('gameid', 'DESC')->get();
				$games_data_more[$key] =  $value;
				$games_data_more[$key]->games =  $games;
			};
		} else {
			$games_data_more = \DB::connection('mysql2')->table('gamelists')->where('active', '=', 1)->orderBy('gameid', 'DESC')->offset($lseqno)->limit(9)->get();
		}
