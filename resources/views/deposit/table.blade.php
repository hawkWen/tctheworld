...
<div class="card">
	<div class="card-body">
		{!! (isset($search_map) ? $search_map : '') !!}
		 ...
		 <?php echo Form::open(array('url'=>'deposit', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
				...
				<tr id="form-0" >
				  ...
				  @endif        
	           		...
	           		<?php foreach ($rowData as $row) : 
	           			  $id = $row->deposit_id;
	           		?>
	                <tr class="editable" id="form-{{ $row->deposit_id }}">
						<td class="thead"> <?php echo ++$i;?>  </td>
						<td >
							<input type="checkbox" class="ids filled-in" name="ids[]" value="{{ $row->deposit_id }}" id="val-{{ $row->deposit_id }}" />
							<label for="val-{{ $row->deposit_id }}"></label>
						</td>					
						@if($setting['view-method']=='expand')
						<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->deposit_id }}" data-url="{{ secure_url('deposit/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
						@endif		
						 ...
						 <?php foreach ($tableGrid as $field) :
							...
							endforeach; 
						  ?>
						   <td data-values="action" data-key="<?php echo $row->deposit_id ;?>"  >
						{!! SiteHelpers::buttonAction('deposit',$access,$id ,$setting) !!}
						{!! SiteHelpers::buttonActionInline($row->deposit_id,'deposit_id') !!}	
							
					</td>			 
	                </tr>
	                @if($setting['view-method']=='expand')
	                <tr style="display:none" class="expanded" id="row-{{ $row->deposit_id }}">
	                	<td class="thead"></td>
	                	<td></td>
	                	<td></td>
