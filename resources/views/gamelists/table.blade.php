...
<div class="card">
	<div class="card-body">
		{!! (isset($search_map) ? $search_map : '') !!}
		 ...
		 <?php echo Form::open(array('url'=>'gamelists', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
				...
				<tr id="form-0" >
				  ...
				  @endif        
	                ...
	                <tr class="editable" id="form-{{ $row->id }}">
						...
						<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ secure_url('gamelists/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
						@endif		
								...
								$value = SiteHelpers::formatRows($row->{$field['field']}, $field , $row);
							 	...
							 	@if(SiteHelpers::filterColumn($limited ))
									 <td align="<?php echo $field['align'];?>" data-values="{{ $row->{$field['field']} }}" data-field="{{ $field['field'] }}" data-format="{{ htmlentities($value) }}" {!! $addClass !!}>					 
										@if($field['field']=='image_large'||$field['field']=='image_normal')
<img src="{!! $value !!}" style="max-height:80px;">
@elseif($field['field']=='partner')
<img src="/uploads/images/partners/{!! $value !!}" style="max-height:80px;">
@else 
          {!! $value !!}  
@endif							 						 
									 </td>
						...
						{!! SiteHelpers::buttonAction('gamelists',$access,$id ,$setting) !!}
