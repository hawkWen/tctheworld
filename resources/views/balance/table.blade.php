...
<div class="card">
	<div class="card-body">
		{!! (isset($search_map) ? $search_map : '') !!}
		 ...
		 <?php echo Form::open(array('url'=>'balance', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
	    ...
	    <table class="table table-hover table-striped " id="{{ $pageModule }}Table">
	        <thead>
				<tr>
					<th width="5" style="width: 5px;"><input type="checkbox" class="checkall filled-in" id="checked-all"  />
				...
				<tr id="form-0" >
				  ...
				  @endif        
	                ...
	                <tr class="editable" id="form-{{ $row->id }}">
						<td width="5" style="width: 5px; text-align: center;" class="thead"> <?php echo ++$i;?>  <br>	<input type="checkbox" class="ids filled-in" name="ids[]" value="{{ $row->id }}" id="val-{{ $row->id }}" />
							<label for="val-{{ $row->id }}"></label></td>
										
						@if($setting['view-method']=='expand')
						<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->id }}" data-url="{{ secure_url('balance/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
						@endif		
						...
						{!! SiteHelpers::buttonAction('balance',$access,$id ,$setting) !!}
