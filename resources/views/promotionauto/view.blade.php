	...
	<div class="card-body">
		...
		<div class="toolbar-nav">
			<div class="row">
				<div class="col-md-6 ">
					<div class="btn-group">
						<a href="{{ secure_url('promotionauto?return='.$return) }}" class="tips btn btn-danger  btn-sm  " title="{{ __('core.btn_back') }}"><i class="fa  fa-times"></i></a>		
						@if($access['is_add'] ==1)
				   		<a href="{{ secure_url('promotionauto/'.$id.'/edit?return='.$return) }}" class="tips btn btn-info btn-sm  " title="{{ __('core.btn_edit') }}"><i class="icon-note"></i></a>
						@endif
					</div>	
				</div>
				<div class="col-md-6 text-right">			
					<div class="btn-group">
				   		<a href="{{ ($prevnext['prev'] != '' ? secure_url('promotionauto/'.$prevnext['prev'].'?return='.$return ) : '#') }}" class="tips btn btn-primary  btn-sm"><i class="fa fa-arrow-left"></i>  </a>	
						<a href="{{ ($prevnext['next'] != '' ? secure_url('promotionauto/'.$prevnext['next'].'?return='.$return ) : '#') }}" class="tips btn btn-primary btn-sm "> <i class="fa fa-arrow-right"></i>  </a>			
					</div>			
				</div>	
		...
		<div class="table-responsive">
			<table class="table  table-bordered " >
				<tbody>	
					...
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Max Bonus', (isset($fields['max_bonus']['language'])? $fields['max_bonus']['language'] : array())) }}</td>
						<td>{{ $row->max_bonus}} </td>
