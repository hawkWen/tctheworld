...
<div class="card">
	<div class="card-body">
		{!! (isset($search_map) ? $search_map : '') !!}
		 ...
		 <?php echo Form::open(array('url'=>'withdraw', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
				...
				<tr id="form-0" >
				  ...
				  @endif        
	           		...
	           		<?php foreach ($rowData as $row) : 
	           			  $id = $row->withdraw_id;
	           		?>
	                <tr class="editable" id="form-{{ $row->withdraw_id }}">
						<td class="thead"> <?php echo ++$i;?>  </td>
						<td >
							<input type="checkbox" class="ids filled-in" name="ids[]" value="{{ $row->withdraw_id }}" id="val-{{ $row->withdraw_id }}" />
							<label for="val-{{ $row->withdraw_id }}"></label>
						</td>					
						@if($setting['view-method']=='expand')
						<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->withdraw_id }}" data-url="{{ secure_url('withdraw/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
						@endif		
						 ...
						 <?php foreach ($tableGrid as $field) :
							...
							endforeach; 
						  ?>
						   <td data-values="action" data-key="<?php echo $row->withdraw_id ;?>"  >
						{!! SiteHelpers::buttonAction('withdraw',$access,$id ,$setting) !!}
						{!! SiteHelpers::buttonActionInline($row->withdraw_id,'withdraw_id') !!}	
							
					</td>			 
	                </tr>
	                @if($setting['view-method']=='expand')
	                <tr style="display:none" class="expanded" id="row-{{ $row->withdraw_id }}">
	                	<td class="thead"></td>
	                	<td></td>
	                	<td></td>
