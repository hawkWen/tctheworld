...
<div class="toolbar-nav">
    <div class="btn-group">
    	<a href="{{ secure_url('sximo/menu/index/0?pos=top')}}" class="btn  btn-sm btn-primary"> <i class="fa fa-plus"></i> new  </a>
        <a href="{{ secure_url('sximo/menu?pos=top')}}" class="btn  btn-sm btn-info"> <i class="fa fa-bars"></i> {{ __('core.tab_topmenu') }}  </a>
        <a href="{{  secure_url('sximo/menu?pos=sidebar') }}" class="btn  btn-sm btn-success post_url"> <i class="fa fa-bars"></i> {{ __('core.tab_sidemenu') }} </a>
    </div>
</div>
							...
							@foreach ($menus as $menu)
								  <li data-id="{{$menu['menu_id']}}" class="dd-item dd3-item">
									<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu['menu_name']}}
										...
										<a href="{{ secure_url('sximo/menu/index/'.$menu['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a></span>
									</div>
											...
											@foreach ($menu['childs'] as $menu2)
											 <li data-id="{{$menu2['menu_id']}}" class="dd-item dd3-item">
												<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{$menu2['menu_name']}}
													<span class="pull-right">
													<a href="{{ secure_url('sximo/menu/index/'.$menu2['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a></span>
												</div>
													...
													@foreach($menu2['childs'] as $menu3)
													 	<li data-id="{{$menu3['menu_id']}}" class="dd-item dd3-item">
															<div class="dd-handle dd3-handle"></div><div class="dd3-content">{{ $menu3['menu_name'] }}
																<span class="pull-right">
																<a href="{{ secure_url('sximo/menu/index/'.$menu3['menu_id'].'?pos='.$active)}}"><i class="ti-pencil"></i></a>
																</span>
															</div>
														</li>	
													@endforeach
		 			...
		 			{!! Form::open(array('url'=>'sximo/menu/save', 'class'=>'form-horizontal','files' => true  , 'parsley-validate'=>'','novalidate'=>' ')) !!}
						  ...
						  <div class="form-group row  " >
							...
							<div class="col-md-8">
							  ...
							  <p> View Icon Codes : 
							  <a href="{{ secure_url('sximo/menu/icon')}}" onclick="SximoModal(this.href,'Select Icon'); return false;"> Browse Icons  </a>  
							 </div> 
						  </div> 					
						  <div class="form-group row  " >
					  ...
					  <div class="form-group row">
						...
						@if($row['menu_id'] !='')
							<button type="button"onclick="SximoConfirmDelete('{{ secure_url('sximo/menu/destroy/'.$row['menu_id'].'?pos='.$active)}}')" class="btn btn-danger btn-sm">  Delete </button>
						@endif	
						</div>	  
