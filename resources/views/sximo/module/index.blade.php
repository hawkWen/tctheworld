...
<div class="toolbar-nav">
    <div class="row">
        <div class="col-md-6">
                ...
                <div class="btn-group">
                    <a  href="{{ secure_url('sximo/module/create')}}" onclick="SximoModal(this.href ,'Create Module'); return false" class="btn  btn-sm btn-primary">  Create Module </a>
                    <a href="{{ secure_url('sximo/module/package')}}" class="btn  btn-sm btn-success post_url"> Backup Module </a>
                    <a href="{{ secure_url('sximo/tables')}}" class="btn  btn-danger btn-sm">  Database Table </a>
                    <a href="{{ secure_url('sximo/server/repository')}}" class="btn btn-warning  btn-sm">  Repositories</a>
        ...
        <div class="col-md-6 text-right">
                ...
                <div class="p-sm m-b unziped" style="display:none; padding: 5px 5px 30px ; margin-bottom:10px;">
                    ...
                    <ul class="nav nav-tabs" style="margin-bottom:10px;">
                        <li  class="nav-item"  ><a class="nav-link @if($type =='addon') active @endif" href="{{ secure_url('sximo/module')}}"> {{ Lang::get('core.tab_installed') }}  </a></li>
                        <li class="nav-item "><a class="nav-link @if($type =='core') active @endif" href="{{ secure_url('sximo/module?t=core')}}"> {{ Lang::get('core.tab_core') }}</a></li>
                    </ul>     
    ...
    <div class="card-body">
            ...
            <tr>        
                ...
                <button class="btn  btn-sm dropdown-toggle" data-toggle="dropdown"> Action </span>
                    ...
                    <ul  class="dropdown-menu icons-right " style="z-index: 999999">
                        @if($type != 'core')
                        <li class="nav-item"><a class="nav-link" href="{{ secure_url($row->module_name)}}"> {{ Lang::get('core.btn_view') }} Module </a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ secure_url('sximo/module/duplicate/'.$row->module_id)}}" onclick="SximoModal(this.href,'Duplicate/Clone Module'); return false;" > Duplicate/Clone </a></li>                       
                        @endif
                        <li class="nav-item"><a class="nav-link" href="{{ secure_url('sximo/module/config/'.$row->module_name)}}"> {{ Lang::get('core.btn_edit') }}</a></li> 
                        
                        @if($type != 'core')
                        <li class="nav-item"><a class="nav-link" href="javascript://ajax" onclick="SximoConfirmDelete('{{ secure_url('sximo/module/destroy/'.$row->module_id)}}')"> {{ Lang::get('core.btn_remove') }}</a></li>
                        <li class="divider"></li>
                        <li class="nav-item"><a class="nav-link"  onclick="SximoModal('{{ secure_url('sximo/module/build/'.$row->module_name)}}','Rebuild Module ')" href="javascript:;"> Rebuild All Codes</a></li>
                        @endif
                    </ul>
                </div>                  
                </td>
        ...
        <br /><br />
        <a href="{{ secure_url('sximo/module/create')}}" onclick="SximoModal(this.href ,'Create Module'); return false" class="btn btn-primary btn-sm "> {{ Lang::get('core.fr_createmodule') }} </a>
         </p>   
        @endif
    </div>
</div>
