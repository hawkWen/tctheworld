...
<div class="card">
	<div class="card-body">
		{!! (isset($search_map) ? $search_map : '') !!}
		 ...
		 <?php echo Form::open(array('url'=>'banks', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
				...
				<tr id="form-0" >
				  ...
				  @endif        
	           		...
	           		<?php foreach ($rowData as $row) : 
	           			  $id = $row->bank_id;
	           		?>
	                <tr class="editable" id="form-{{ $row->bank_id }}">
						<td class="thead"> <?php echo ++$i;?>  </td>
						<td >
							<input type="checkbox" class="ids filled-in" name="ids[]" value="{{ $row->bank_id }}" id="val-{{ $row->bank_id }}" />
							<label for="val-{{ $row->bank_id }}"></label>
						</td>					
						@if($setting['view-method']=='expand')
						<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->bank_id }}" data-url="{{ secure_url('banks/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
						@endif		
						 ...
						 <?php foreach ($tableGrid as $field) :
							...
							endforeach; 
						  ?>
						   <td data-values="action" data-key="<?php echo $row->bank_id ;?>"  >
						{!! SiteHelpers::buttonAction('banks',$access,$id ,$setting) !!}
						{!! SiteHelpers::buttonActionInline($row->bank_id,'bank_id') !!}	
							
					</td>			 
	                </tr>
	                @if($setting['view-method']=='expand')
	                <tr style="display:none" class="expanded" id="row-{{ $row->bank_id }}">
	                	<td class="thead"></td>
	                	<td></td>
	                	<td></td>
