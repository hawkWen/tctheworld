	...
	<div class="col-md-8 text-right"> 	
		...
		<div class="btn-group">
			 @if($access['is_add'] ==1)
				...
				<a href="{{ secure_url('cms/posts/create?return='.$return) }}" class="btn btn-info btn-sm"  
			...
			<div class="btn-group">
		        ...
		        <ul class="dropdown-menu">
		         @if($access['is_excel'] ==1)
					<li><a href="{{ secure_url( $pageModule .'/export?do=excel&return='.$return) }}" class="nav-link"> Export CSV </a></li>	
				@endif
				@if($access['is_add'] ==1)
					<li class="nav-item"><a class="nav-link" href="{{ secure_url($pageModule .'/import?return='.$return) }}" onclick="SximoModal(this.href, 'Import CSV'); return false;"> Import CSV</a></li>
					<li class="nav-item"><a class="nav-link" href="javascript://ajax" class=" copy " title="Copy" > Copy selected</a></li>
				@endif	
					<li class="nav-item"><a class="nav-link" href="{{ secure_url($pageModule) }}"  > Clear Search </a></li>
						 ...
						 <td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ secure_url('posts/read/'.$row->alias)}}" target="_blank" class="tips nav-link" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ secure_url('cms/posts/'.$row->pageID.'/edit?return='.$return) }}" class="tips nav-link" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
