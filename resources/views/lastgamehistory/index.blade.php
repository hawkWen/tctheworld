...
<div class="page-titles">
  <h2> {{ $pageTitle }} <small> {{ $pageNote }} </small></h2>
	...
	<div id="lastgamehistoryView"></div>	
	<div id="lastgamehistoryGrid">
		<div class="card">
			<div class="card-body">
			...
			<div class="table-responsive">			
				<table id="lastgamehistoryTable" class="display table  table-striped table-hover" cellspacing="0" width="100%">
			        <thead>
			            <tr>
			            	<th>ID</th>	
	...
	$(document).ready(function() {
		...
		var rows_selected = []; 			
	   	var table = $('#lastgamehistoryTable').DataTable( {
	        ...
	        "ajax": {
	            "url": "{!! secure_url('lastgamehistory')!!}",
	            "type": "POST"
		...
		$('#lastgamehistoryTable tbody').on('click', 'td.details-control', function () {
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            tr.addClass('shown');
	            $.get('{{ secure_url("lastgamehistory/")}}/'+id , function(callback){
	            	$('#'+id).html(callback);
	            	$('#'+id).addClass('data');
	            })
	    ...
	    $('#lastgamehistoryTable').Sdtable({
	    	tableId : '#lastgamehistory',
	    	table   : table,
	    	action  : '{{ secure_url("lastgamehistory")}}' 
	    });
