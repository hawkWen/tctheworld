...
var scoreBoardTitle = 'อันดับคะแนนกิจกรรมเครดิตฟรี'; //text for scoreboard title
...
var userIDType = 'mobile'; //user ID type 'email' or 'mobile' field
var mobileFormat = {
						matches:'9999999999', //mobile format
						minLength:10, //mobile min length
						maxLength:10 //mobile max length
};

var enableLevel = false; //enable to display game stage/level in scoreboard list
var scoreReverse = false; //reverse scoreboard list in descending
var scoreListingFormat = ''; //return score in daily, weekly or monthly, empty string for all time (daily, weekly, monthly)
...
var topScore_arr = [];
if(!enableLevel){
	topScore_arr = [{col:'อันดับ', percentX:17, align:'center'},
				{col:'เบอร์', percentX:30, align:'left'},
				{col:'ด่านที่ชนะ', percentX:82, align:'center'}];
}else{
...
function addScoreboardAssets(){
	if(!fixedScreen){
		if(typeof viewport != 'undefined'){
		    manifest.push({src:'assets/scoreboard/bg_scoreboard_portrait.png', id:'bgScoreboardP'});
...
function buildScoreBoardCanvas(){
	...
	$.get('submit.html', function(data){
		...
		if(userIDType == 'email'){
			...
			$('.forEmail').hide();
			$('.forMobile').show();
			$('#uMobile').attr('placeholder', mobileFormat.matches);
			$('#uMobile').attr('maxlength', mobileFormat.maxLength);
		}
...
function submitUserScore(score, type){
	...
	if(errorCon){
		...
		$.ajax({
		  ...
		  data: { name: $('#uName').val(), email: userIDValue, type:type, score:score, enableLevel:enableLevel, format:scoreListingFormat},
...
function loadScoreboard(type){
	...
	$.ajax({
	  type: "POST",
	  url: 'topRank.php',
	  data: {type:type, reverse:scoreReverse, limit:pageLimit, format:scoreListingFormat},
			...
			for(var i=0; i<scoreList.length; i++){
				if(typeof scoreList[i] != "undefined"){
					...
					if(typeof millisecondsToTime == 'function' ) { 
						scoreNum = scoreList[i].score;
					}
					if(enableLevel){
						$.scoreList[(i+1)+'_'+1].text = scoreList[i].email;
						$.scoreList[(i+1)+'_'+2].text = scoreList[i].type;

						$.scoreList[(i+1)+'_'+3].text = scoreNum;
					}else{
						$.scoreList[(i+1)+'_'+1].text = scoreList[i].email;
						$.scoreList[(i+1)+'_'+2].text = scoreNum;	
					}
				}
			}
		}
