...
if ($batch_id && $_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if($update_status == 'remove'){
		$status_message = ' user score removed successfully!';
		...
		$status_message = ' user added to filter successfully!';
		foreach ($batch_id as &$scoreboard_id) {
			$db->where('id', $scoreboard_id);
	...
	$_SESSION['info'] = $completeRecord." of ".$totalRecord.$status_message;
	header('location: scoreboard.php');
	exit;
}