...
if(strpos($jsonObject->message,"บ.")){
  ...
  $amount = GetStringBetween($jsonObject->message, "ได้รับ", "บ.");

  $amount = str_replace(',', '', $amount);


  $fullnamex = GetStringBetween($jsonObject->message, "จาก", "ยอดคงเหลือ");

 // $query = $mysqli->query("select username from tb_users where phone = '".$phone."'");
  $fullname = explode(" ", trim($fullnamex));


  $queryx = $mysqli->query("select * from deposit where bankweb_id = 2 and details = '".$jsonObject->message."'");
  $check_depositdup = $queryx->num_rows;
    ...
    $query = $mysqli->query("select * from tb_users where first_name = '".$fullname[0]."' and last_name = '".$fullname[1]."' and active = 1");
    ...
    $check_user = $query->num_rows;
         ...
         $mysqli->query("INSERT INTO deposit(user_id, amount, status, bankweb_id, trans_id, ac_number, channel, details, date_time, deposit_mode, deposit_hold) VALUES ('".$userid."','".$amount."','".$status."','2','".date("Y-m-d H:i")."','".$fullnamex."','truewallet','".$jsonObject->message."','".date("Y-m-d H:i:s")."','auto',".$deposit_hold.")");
...
if($jsonObject->subject=='027777777'){
  ...
  $date = substr($jsonObject->message, 0, 5);
                    $time = GetStringBetween($jsonObject->message, "ได้รับ", " ");
