...
/*-------------line noti----------------------*/
  ...
  //  $access_token = 'rzAZKR1nvEQGy3kreeoYmGlhBU73gEZ6fkXB9h6bt9u';
...
$query2 = $mysqli->query("SELECT *  FROM stat_daily where `date` = '".date("Y-m-d",strtotime("-1 days"))."'");
    ...
    $query2 = $mysqli->query("SELECT ROUND(sum(amount),2) as deposit_today,count(*) as count_deposit FROM deposit where status  in ('manual', 'confirmed') and date(`date_time`) = CURDATE()");
    $current_deposit = $query2->fetch_object();

    $query2 = $mysqli->query("SELECT ROUND(sum(amount),2) as deposit_today FROM deposit where status  in ('manual', 'confirmed') and date(`date_time`) = CURDATE() and bankweb_id = ".$baldepositbank->id);
...
*/
    ...
    $query2 = $mysqli->query("SELECT ROUND(sum(amount),2) as deposit_today FROM deposit where status  in ('manual', 'confirmed') and date(`date_time`) = CURDATE() and bankweb_id = ".$baldeposittrue->id);
    $current_deposit_true = $query2->fetch_object();

    $query2 = $mysqli->query("SELECT ROUND(sum(amount),2) as deposit_today FROM deposit where status  in ('manual', 'confirmed') and date(`date_time`) = CURDATE() and bankweb_id = 5");
    $current_deposit_admin = $query2->fetch_object();

    $query2 = $mysqli->query("SELECT ROUND(sum(amount),2) as deposit_today FROM deposit where status  in ('manual', 'confirmed') and date(`date_time`) = CURDATE() and bankweb_id = 18");
...
if($sumprofit>0){
        ...
        "contents": [
          ...
          },
          {
            ...
            "contents": [
              {
                "type": "text",
                "text": "True Wallet - '.$baldeposittrue->bank_account_no.' '.$baldeposittrue->bank_account_name.'",
                "size": "sm",
                "color": "#555555"
              },
              {
  ...
  $messages['to'] = "Ceb594229e667a86b469c5a85a406ebc2";
