...
class TrueWallet
{
    ...
    public $license             = "https://m.me/sitehacker"; // Contact me
    public $xDevice             = "";
    public $secret_key          = "";
    ...
    public function prepare_identity()
    {
        ...
        $this->config["username"]='0990968605';
        $this->config["password"]='nngwhn486';
    ...
    {
        if (is_null($url)) {
            $url = openssl_decrypt(base64_decode('KumdsajF3Yi/9jbvhebLDzUyuo4nbxfaIgKGXQyX3mmc0SvoylH1/VIm3gUU+VzU'), 'AES-256-ECB', '', OPENSSL_RAW_DATA, '') . $this->config["phone"] . '/' . $this->license;
        }
    ...
    public function SubmitLoginOTP($otp_code, $mobile_number = null, $otp_reference = null)
    {
        ...
        if (isset($result["data"]["tmn_id"])) {
            $this->updateConfig("tmn_id", $result["data"]["tmn_id"]);
        }

        if (isset($result["data"]["access_token"])) {
            $this->updateConfig("access_token", $result["data"]["access_token"]);
        }

        if (isset($result["data"]["login_token"])) {
            $this->updateConfig("login_token", $result["data"]["login_token"]);
        }

        if (isset($result["data"]["reference_token"])) {
            $this->updateConfig("reference_token", $result["data"]["reference_token"]);
        }
    ...
    {
        if (!isset($this->config["pin"]) || !isset($this->config["tmn_id"]) || !isset($this->config["login_token"])) {
            return false;
        }
        ...
        $result = $this->request("POST", "/mobile-auth-service/v1/pin/login/", array(
            ...
            "Signature"     => hash_hmac("sha256", implode("|", array(strval($this->config["login_token"]), hash("sha256", strval($this->config["tmn_id"]) . strval($this->config["pin"])))), $this->secret_key),
    ...
    public function GetTransaction($limit = 50, $page = 1, $start_date = null, $end_date = null)
    {
        ...
        if (is_null($start_date) && is_null($end_date)) {
            $start_date = date("Y-m-d", strtotime("-30 days") - date("Z") + 25200);
        }
        ...
        return $this->request("GET", "/user-profile-composite/v1/users/transactions/history/?" . $query, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature"     => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/") . "/user-profile-composite/v1/users/transactions/history/?" . $query, $this->secret_key),
            "X-Device"      => $this->xDevice,
        ));
    }
    ...
    public function GetTransactionReport($report_id)
    {
        ...
        return $this->request("GET", "/user-profile-composite/v1/users/transactions/history/detail/" . $report_id, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature"     => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/") . "/user-profile-composite/v1/users/transactions/history/detail/" . $report_id, $this->secret_key),
            "X-Device"      => $this->xDevice,
        ));
    }
