...
<div class="container mt-4">
          ...
          $m = $_GET['m'];
            ...
            <div class="card casino-card">
                <div class="card-body text-center">
                  <h3 style="color: khaki"><i class="fa fa-cog" aria-hidden="true"></i>&nbsp;สูตรที่ <?= $m ?> Allbet</h3>
  ...
  <tbody>
      <?php 
if(isset($_GET['rm'])){
    $Query = query("DELETE FROM fml_allbet WHERE id = ?",array($_GET['rm']));
?>
            <script>Swal.fire(
      'สำเร็จ!',
      'ลบข้อมูลแล้ว',
    ...
    window.location.href = "?page=m_formula_allbet&m=<?= $m ?>";

		});</script>
                             <?php
}
      $Query = query("SELECT * FROM fml_allbet WHERE idea = ?",array($m));
      while($Data = $Query->fetch()){
          ...
          <td>
            <a class="btn btn-side" href="?page=m_formula_allbet&m='.$m.'&rm='.$Data['id'].'"><i class="fa fa-times"></i> ลบ</a>
          </td>  
          ';
          echo '</tr>';
      }
      ?>
        ...
        if(isset($_POST['add'])){
          ...
          $m = $_GET['m'];
            ...
            $fm .= $_POST['fm4'];
            // $fm .= $_POST['fm5'];
           $Query = query("INSERT INTO fml_allbet (fm,an,idea) VALUES (?,?,?)",array($fm,$_POST['an'],$m));
            ?>
            <script>Swal.fire(
      'สำเร็จ!',
      'เพิ่มข้อมูลแล้ว',
...
<!--             <div class="card casino-card mt-4">
                <div class="card-body text-center">
                <select class="browser-default custom-select mb-3" onchange="window.location.href = '?page=m_formula_allbet&m='+this.options[this.selectedIndex].value; ">
  <option selected>เลือกสูตร</option>
                    <?php 
                    $Query = query("SELECT * FROM fml_list_allbet");
                    while($Data = $Query->fetch()){
...
<a href="?page=m_formula&add" class="btn btn-side btn-block">
    เพิ่มสูตร
                    ...
                    <?php 
                    if(isset($_GET['add'])){
                        $Query = query("INSERT INTO fml_list_allbet (time) VALUES (?)",array(time()));
    ...
    window.location.href = "?page=m_formula_allbet";
