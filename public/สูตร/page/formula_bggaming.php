<?php
  ...
  $post = "";
  $response = $bypass->curl_post($GLOBALS['bggaming_genkey'],$post);
...
function ez(){
  // $.get("<?php echo $url_base_casino; ?>encryption/api_casino_encode.php?g=bggaming&key=<?php echo $decode; ?>")
 $.get( "<?php echo $url_bg.$decode; ?>")  
  .done(function( data ) {
  


  room1_name = data['data'][<?= $r ?>]['room_id'];
...
// room1 = data[<?= $r ?>]['data'];
  ...
  $.get( "fml_bggaming.php?fm="+oil+"&idea=1&room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
    console.log(data.fm);
    $("#per").html(<?= rand(70,100) ?> + "%");
...
//alert(text);
  ...
  $.get( "ajax/getscore_bggaming.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
  $("#table_score").html(data);
          });
  
  $.get( "ajax/chart_bggaming.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
...
function load_chart_data() {
    $.ajax({
        url: "ajax/chart_bggaming.php?room="+<?= $_GET['room'] ?>, 
        type: 'GET',
        data: {get_chart: true},
        dataType: 'JSON', // 
