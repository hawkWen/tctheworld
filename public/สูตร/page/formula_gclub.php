...
//
  ...
  $.get( "fml_gclub.php?fm="+oil+"&idea=1&room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
    console.log(data.fm);
    $("#per").html(<?= rand(70,100) ?> + "%");
...
//alert(text);
	...
	$.get( "ajax/getscore_gclub.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
	$("#table_score").html(data);
					});
	
	$.get( "ajax/chart_gclub.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
...
function load_chart_data() {
    $.ajax({
        url: "ajax/chart_gclub.php?room="+<?= $_GET['room'] ?>, 
        type: 'GET',
        data: {get_chart: true},
        dataType: 'JSON', // 
