<?php
  ...
  $post = "";
  $response = $bypass->curl_post($GLOBALS['dg_genkey'],$post);
...
<script>
  ...
  ez();
  var bac = window.setInterval(ez, 500);

function ez(){
  //$.get( "<?php echo $url_base_casino; ?>/encryption/api_casino.php?g=Dreamgaming")
  // $.get("<?php echo $url_base_casino; ?>encryption/api_casino_encode.php?g=dg&key=<?php echo $decode; ?>")
$.get( "<?php echo $url_dg.$decode; ?>")
  .done(function( data ) {
...
//
  ...
  $.get( "fml_dg.php?fm="+oil+"&idea=1&room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
    console.log(data.fm);
    $("#per").html(<?= rand(70,100) ?> + "%");
...
//alert(text);
  ...
  $.get( "ajax/getscore_dg.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
  $("#table_score").html(data);
          });
  
  $.get( "ajax/chart_dg.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
...
function load_chart_data() {
    $.ajax({
        url: "ajax/chart_dg.php?room="+<?= $_GET['room'] ?>, 
        type: 'GET',
        data: {get_chart: true},
        dataType: 'JSON', // 
