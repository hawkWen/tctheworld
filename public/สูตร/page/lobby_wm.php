<?php
  ...
  $post = "";
  $response = $bypass->curl_post($GLOBALS['wm_genkey'],$post);
...
<section id="headers">
        <div class="headers-container">
            ...
            <h2 class="animated fadeIn"><?php echo $Acc1["text_headers2"] ?></h2>
        </div>
    </section>
...
for ($x = 0; $x <= 30;) {
  ...
  <div class="m-1">
    <a style="text-decoration:none;" href="?page=formula_wm&room='.($x+1).'">
      <div class="row resroom frame-casino" style="background-image: url(img/frame.png);">
      <div class="col wm">
          
        </div>
        <div class="col-6 col-md-5 text-center tab">
          <div class="chancetxt_wm">ชื่อห้อง</div>
          <span class="showtext_wm" style="color: khaki;" id="per'.$x.'">
            '.($x+1).'
          </span>
        </div>
      </div>
...
ez();
  ...
  function ez(){
  // $.get("<?php echo $url_base_casino; ?>encryption/api_casino_encode.php?g=wm&key=<?php echo $decode; ?>")
$.get( "<?php echo $url_wm.$decode; ?>") 
  .done(function( data ) {
  console.log(data);  
...
for (i = 0; i < loop; i++) {
   ...
   if(data['data'][i]['room_name'] != ""){
    $("#per"+i).html(data['data'][i]['room_name']);
   }else{
    $("#per"+i).html('<a>ปรับปรุง</a>');
   }
}
  });
