<?php
  ...
  $post = "";
  $response = $bypass->curl_post($GLOBALS['sexy_genkey'],$post);
...
function ez(){
  // $.get("<?php echo $url_base_casino; ?>encryption/api_casino_encode.php?g=sexy&key=<?php echo $decode; ?>")
$.get( "<?php echo $url_sexy.$decode; ?>")
  .done(function( data ) {
  ...
  const obj = data
  console.log(obj['data'][<?= $r ?>]['records']);


  //console.log(data);
	room1_name = obj['data'][<?= $r ?>]['table_name'];
  //console.log(room1_name);


	room1 = obj['data'][<?= $r ?>]['records'];

	status = obj['data'][<?= $r ?>]['status'];
	console.log(status);
...
////
  ...
  $.get( "fml_sexy.php?fm="+oil+"&idea=1&room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
    console.log(data.fm);
     $("#per").html(<?= rand(70,100) ?> + "%");
...
//alert(text);
  ...
  .fail(function(){
	...
	$.get( "ajax/getscore_sexy.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
	$("#table_score").html(data);
					});
	
  $.get( "ajax/chart_sexy.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
...
function load_chart_data() {
    $.ajax({
        url: "ajax/chart_sexy.php?room="+<?= $_GET['room'] ?>, 
        type: 'GET',
        data: {get_chart: true},
        dataType: 'JSON', // 
