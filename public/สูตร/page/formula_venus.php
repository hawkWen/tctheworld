<?php
  ...
  $post = "";
  $response = $bypass->curl_post($GLOBALS['venus_genkey'],$post);
...
function ez(){
	//$.get( "<?php echo $url_base_casino; ?>/encryption/api_casino.php?g=Venus")
  // $.get("<?php echo $url_base_casino; ?>encryption/api_casino_encode.php?g=venus&key=<?php echo $decode; ?>")
$.get( "<?php echo $url_venus.$decode; ?>")  
  .done(function( data ) {
...
//room1_name = data[<?= $r ?>]['name'];
  room1_name = data['data'][<?= $r ?>]['table_name'];
...
//
  ...
  $.get( "fml_venus.php?fm="+oil+"&idea=1&room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
    console.log(data.fm);
    $("#per").html(<?= rand(70,100) ?> + "%");
...
//alert(text);
  ...
  .fail(function(){
	...
	$.get( "ajax/getscore_venus.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
	$("#table_score").html(data);
					});
	
	$.get( "ajax/chart_venus.php?room="+<?= $_GET['room'] ?>)
  .done(function( data ) {
...
function load_chart_data() {
    $.ajax({
        url: "ajax/chart_venus.php?room="+<?= $_GET['room'] ?>, 
        type: 'GET',
        data: {get_chart: true},
        dataType: 'JSON', // 
