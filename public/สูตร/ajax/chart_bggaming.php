...
$Query1 = query("SELECT id , user , room , date , result , count(*) as number FROM statisctic_bggaming WHERE room = ? AND date = ? AND user = ? AND result != 'cancel' AND result != 'wait' AND result != 'tle1' GROUP BY result ORDER BY id DESC",array($_GET['room'],date("d-m-Y"),$_SESSION['user']));
