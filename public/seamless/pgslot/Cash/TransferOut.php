...
if(strpos($req_data['player_name'],"xmrdb")&&(float) $req_data['transfer_amount'] > 49)
{
     echo json_encode([
                'data' => null,
                'error' => array(
    ...
    if ($validate_operator_player_session['error'] != null) {
        ...
        if($querytx)
        {
            ...
            if($querytx->num_rows > 0) {
                ...
                if ((float) $bal_remain < (float) $req_data['transfer_amount']) {
                    echo json_encode([
                        'data' => null,
                        'error' => array(
                            'code' => 3202,
...
/*$validate_player_name = validatePlayerName($req_data);
...
/*if (isTrxDuplicate($req_data['transaction_id'], 'TransferOut')) {  // Check transaction_id duplicate
    echo json_encode([
        'data' => array(
            'currency_code' => $currency,
            'balance_amount' => $member_wallet['main_wallet'],
            'updated_time' => $req_data['updated_time'],
            'duplicated_trx_id' => true
        ),
        'error' => $errorCode
    ]);
    exit();
    ...
    //adjustMemberTurnover($req_data['player_name'], $req_data['transfer_amount'], 'pgs', 2);  //###############

    //adjustAFF($req_data['player_name'], $req_data['transfer_amount'], 1);

    //$req_data_ori = pg_trx_record('TransferOut', $req_data_ori);
